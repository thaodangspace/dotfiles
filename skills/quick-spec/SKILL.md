---
name: quick-spec
description: "Lightweight spec-driven workflow for small-scope tasks (bug fixes, minor features, refactors, config changes): quick plan → execute → verify"
---
...
All documents live under `~/code/brain/spec/YYYYMMDD/` where `YYYYMMDD` is today's date (infer from context or ask if unsure). Number them sequentially.

Given a title (lowercase, snake case), the files are:

- **Task plan**: `<weight>_task_<title>.md`
...
- `~/code/brain/spec/20260529/01_task_fix_login_rate_limiter_bug.md`
- `~/code/brain/spec/20260529/01_digest_fix_login_rate_limiter_bug.md`

Determine the title from the task description `$ARGUMENTS`. Convert it to lowercase, snake case form. Create the directory if it does not exist.
...
2. **Run gitingest** for each external reference:
   ```bash
   gitingest <url-or-path> -o ~/code/brain/spec/YYYYMMDD/<weight>_digest_<ref-title>.txt
   ```
   - `<ref-title>` is a short, hyphenated slug derived from the repo/library name (e.g., `tanstack-router`, `laravel-reverb`)
   - If the task title already has a digest file, use the same title: `<weight>_digest-<title>.txt`

3. **Read the digest**: After generation, read `<weight>_digest_<ref-title>.txt` to absorb:
...
Create the file at `~/code/brain/spec/YYYYMMDD/<weight>_task_<title>.md` (create the directory if needed).

```markdown
---
title: [Title]
tags: ["language", "framework", "architecture aspects", "topic keywords"]
weight: <weight>
---
...
4. **Iterate**: If I request changes, update the implementation AND update `<weight>_task_<title>.md` if the changes affect the plan. Keep the doc in sync.
...
- **Document is living**: If implementation reveals new information, update `<weight>_task_<title>.md` immediately.
