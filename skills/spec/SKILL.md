...
- **Implementation plan**: `<weight>_impl_<title>.md`
...
- `~/code/brain/spec/20260529/01_spec_fix_login_rate_limiter_bug.md`
- `~/code/brain/spec/20260529/01_impl_fix_login_rate_limiter_bug.md`
...
Create the file at `~/code/brain/spec/YYYYMMDD/<weight>_spec_<title>.md` (create the directory if needed).
...
After the spec is approved, create `~/code/brain/spec/YYYYMMDD/<weight>_impl_<title>.md`.
...
5. **Iterate**: If I request changes, update the implementation AND update `<weight>_spec_<title>.md` / `<weight>_impl_<title>.md` if the changes affect the spec or plan. Keep all documents in sync.
...
- **Documents are living**: If a phase reveals new information, update `<weight>_spec_<title>.md` and `<weight>_impl_<title>.md` immediately. Nothing worse than stale docs.
